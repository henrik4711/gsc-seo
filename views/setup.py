...
def render():
    ...
    st.markdown(
        "<p style='color:#6b6b8a; margin-bottom:2rem;'>Konfigurer Google Search Console forbindelse og API-nogler</p>",
    ...
    with col1:
        ...
        if "Demo" in connect_method:
            ...
            if st.button("Aktiver Demo-tilstand", type="primary"):
                ...
                st.success(f"Demo-data indlaest: {len(demo_df)} raekker")
                st.rerun()
            ...
            if has_env_creds and "gsc_credentials" in st.session_state:
                ...
                2. Opret noegle (JSON format)<br>
                3. Tilfoej service account email til GSC under "Indstillinger &rarr; Brugere og tilladelser"<br>
                4. Upload JSON-filen herunder ELLER saet GSC_CREDENTIALS_JSON env var i Railway
                </div>
                """, unsafe_allow_html=True)
        ...
        if os.environ.get("ANTHROPIC_API_KEY"):
            st.success("API-noegle loaded from environment variable (ANTHROPIC_API_KEY)")
        else:
            api_key = st.text_input(
                "Anthropic API Key",
                type="password",
                value=st.session_state.get("anthropic_key", ""),
                help="Bruges til meta-generering og content-forslag. Saet ANTHROPIC_API_KEY env var i Railway."
            )
            if api_key:
                st.session_state["anthropic_key"] = api_key
                st.success("API-noegle gemt")
        ...
        default_context = (
            "Mshop er en svensk/dansk webshop for voksenprodukter med 40+ ars historie. "
            "Vi saelger vibratorer, dildoer, lingeri, glidmidler og SM-produkter. "
            "Tone: diskret, professionel, imoedekommende. USPs: Fri frakt, hurtig levering, diskret forsendelse."
        )
        ...
        site_context = st.text_area(
            ...
            height=100,
            help="AI'en bruger dette til at skraeddersy meta-tekster og content. Kan saettes via SITE_CONTEXT env var."
        )

        lang_options = ["Swedish", "Danish", "Norwegian", "English"]
        current_lang = st.session_state.get("content_language", "Swedish")
        lang_idx = lang_options.index(current_lang) if current_lang in lang_options else 0

        language = st.selectbox(
            "Primaert sprog for genereret indhold",
            lang_options,
            index=lang_idx
        )
    ...
    with col2:
        ...
        status_row("Claude AI", ai_ready, "Klar" if ai_ready else "Mangler noegle")
        ...
        if gsc_connected:
            ...
            <div style="background:#12121f; border:1px solid #1e1e2e; border-radius:8px; padding:1rem; margin-top:1rem;">
                <div style="font-family:'IBM Plex Mono',monospace; font-size:0.7rem; color:#6b6b8a; margin-bottom:0.5rem;">NOEGLETAL</div>
        ...
        <div style="background:#0d0d15; border:1px solid #1a1a2e; border-radius:8px; padding:1rem;">
            ...
            <div style="font-size:0.8rem; color:#9b9bb8; line-height:1.8;">
                ...
                4. Content Generator &rarr; AI-tekst<br>
                5. Action Plan &rarr; prioriteret liste
            </div>
        </div>
        """, unsafe_allow_html=True)
